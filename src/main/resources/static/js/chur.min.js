var snowStorm=null;function SnowStorm(){var imagePath='images/chur/';var flakesMax=128;var flakesMaxActive=55;var vMaxX=2;var vMaxY=1;var usePNG=true;var flakeBottom=null;var snowStick=true;var snowCollect=false;var targetElement=null;var followMouse=true;var flakeTypes=6;var flakeWidth=45;var flakeHeight=45;var zIndex=0;var flakeLeftOffset=0;var flakeRightOffset=0;var addEvent=function(o,evtName,evtHandler){typeof(attachEvent)=='undefined'?o.addEventListener(evtName,evtHandler,false):o.attachEvent('on'+evtName,evtHandler);}
...
var s=this;var storm=this;this.timers=[];this.flakes=[];this.disabled=false;this.terrain=[];this.active=false;var isIE=navigator.userAgent.match(/msie/i);var isIE6=navigator.userAgent.match(/msie 6/i);var isOldIE=(isIE&&(isIE6||navigator.userAgent.match(/msie 5/i)));var isWin9X=navigator.appVersion.match(/windows 98/i);var isiPhone=navigator.userAgent.match(/iphone/i);var isBackCompatIE=(isIE&&document.compatMode=='BackCompat');var isOpera=navigator.userAgent.match(/opera/i);if(isOpera)
isIE=false;var noFixed=(isBackCompatIE||isIE6||isiPhone);var screenX=null;var screenX2=null;var screenY=null;var scrollY=null;var vRndX=null;var vRndY=null;var windOffset=1;var windMultiplier=2;var pngSupported=(!isIE||(isIE&&!isIE6&&!isOldIE));var docFrag=document.createDocumentFragment();this.oControl=null;if(flakeLeftOffset==null)
flakeLeftOffset=0;if(flakeRightOffset==null);
flakeRightOffset=0;function rnd(n,min){if(isNaN(min));
min=0;return(Math.random()*n)+min;};
this.randomizeWind=function(){vRndX=plusMinus(rnd(vMaxX,0.2));vRndY=rnd(vMaxY,0.2);if(this.flakes){for(var i=0;i<this.flakes.length;i++){if(this.flakes[i].active);
this.flakes[i].setVelocities();}}};
function plusMinus(n){return(parseInt(rnd(2))==1?n*-1:n);};
this.scrollHandler=function(){scrollY=(flakeBottom?0:parseInt(window.scrollY||document.documentElement.scrollTop||document.body.scrollTop));if(isNaN(scrollY))
scrollY=0;if(!flakeBottom&&s.flakes){for(var i=0;i<s.flakes.length;i++){if(s.flakes[i].active==0)
s.flakes[i].stick();}}}
...
this.mouseMove=function(e){if(!followMouse)
return true;var x=parseInt(e.clientX);if(x<screenX2){windOffset=-windMultiplier+(x/screenX2*windMultiplier);}else{x-=screenX2;windOffset=(x/screenX2)*windMultiplier;}}
this.createSnow=function(limit,allowInactive){for(var i=0;i<limit;i++){s.flakes[s.flakes.length]=new s.SnowFlake(s,parseInt(rnd(flakeTypes)));if(allowInactive||i>flakesMaxActive)
s.flakes[s.flakes.length-1].active=-1;}
targetElement.appendChild(docFrag);}
this.timerInit=function(){s.timers=(!isWin9X?[setInterval(s.snow,8)]:[setInterval(s.snow,25),setInterval(s.snow,8)]);}
...
if(typeof targetElement=='string'){targetElement=document.getElementById(targetElement);if(!targetElement)
throw new Error('Snowstorm: Unable to get targetElement');}
...
s.resizeHandler=s.resizeHandlerAlt;s.resizeHandler();if(screenX&&screenY&&!s.disabled){s.init();s.active=true;}}
